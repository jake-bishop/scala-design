...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, moves) #:: initialTail => {
      val currentExplored = explored + block
      val currentNeighbors = newNeighborsOnly(neighborsWithHistory(block, moves), currentExplored)
      currentNeighbors ++ from(initialTail ++ currentNeighbors, currentExplored)
    }
  }
